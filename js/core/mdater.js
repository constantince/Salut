(function($){
	$.fn.mdater = function(config){
		...
		var mdater = {
			...
			getDateStr : function(y, m, d, today){
				var dayStr = '';
				...
				for(var j=startDay; j<=endDay; j++){
					...
					if((this.value.year + '-' + this.value.month + '-' + j) === today){
						aud = '今天';
					}
					dayStr += '<li class="'+current+'" data-day="'+j+'">'+aud+'</li>';
				}
			...
			updateDate : function(add){
				...
				newDateArea.html(this.getDateStr(this.value.year, this.value.month, this.value.date, today));
			...
			refreshView : function(){
				...
				if(initVal){
					...
					date = new Date(arr[0], arr[1]-1 , arr[2].split(' ')[0]);
				}
				else{
					date = new Date();
				}
				...
				var ajd = new Date();
				today = ajd.getFullYear() + '-' + ajd.getMonth()+ '-' + ajd.getDate();
				$('.yeartag').text(y).data('year', y);
				$('.monthtag').text(F.getMonth(m)+'月').data('month', m);
				var dayStr = this.getDateStr(y, m, d, today);
				$('.md_datearea').html(dayStr);
				if($('#Calendar li').length == 0) this.loadLi();
			},
			...
			initListeners : function(){
				...
				input.on('tap', function(){
					...
					if($('.md_mask').length>0 && $('.md_mask').hasClass('show') ){
						_this._hidePanel();
					}else{
						_this.renderHTML();
						var panel = $('.md_panel'),
						mask = $('.md_mask');
						! _this.hasEventBinded && _this.afterShowPanel(mask,panel);
						setTimeout(function () {
							_this._showPanel();
						}, 50);
					}
			...
			loadLi: function(data) {
				var timelimit = ['09:00', '09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00','18:30','19:00','19:30','20:00','20:30','21:00','21:30','22:00','22:30','23:00','23:30'];
				var str = '';
				for(var i=0, l=timelimit.length; i<l; i++) {
					str += '<li>'+ timelimit[i] +'</li>';
				}
				$('#Calendar').html(str);
			},
			afterShowPanel : function (mask,panel) {
				var _this = this;
				_this.hasEventBinded = 1;
				...
				panel.delegates({
					...
					'#Calendar li': function() {
						$(this).parent().find('li').removeClass('active');
						$(this).addClass('active');
						_this.value.time = $(this).html();
						...
						var vdate = _this.value.year + '-' + monthValue + '-' + dateValue+' ' + _this.value.time;
						_this.input.val(vdate);
						_this._hidePanel();
						option.callback && option.callback(vdate);
					...
					'.md_ok' : function(){
						...
						_this.input.val(_this.value.year + '-' + monthValue + '-' + dateValue+' ' +_this.value.time);
						_this._hidePanel();
					},
					'.md_cancel' : function(){
						_this._hidePanel();
